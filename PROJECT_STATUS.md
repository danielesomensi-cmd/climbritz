...
## 🗓️ Ultimo Aggiornamento: 7 Maggio 2026
...
| A020 Clerk auth integration | ✅ Done | Replaces legacy custom-JWT auth. 16 commits on feat/a019-clerk-auth. iOS WKAppBoundDomains + Capacitor allowNavigation set up for Clerk widget in WebView (B-medium). **A019.16:** ALL pages require login except `/sign-in`, `/sign-up`, `/privacy`, `/debug` — Discovery still free tier (no payment) but no longer anonymous (per-user logging in A021 needs guaranteed user_id). Pending: Phase 4 device verification, Railway/Vercel env var setup, merge. |
