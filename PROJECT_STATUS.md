...
## 🗓️ Ultimo Aggiornamento: 8 Maggio 2026
...
| A020 Clerk auth integration | ✅ Merged to main | Replaces legacy custom-JWT auth. 18 commits, merged via PR #1 (commit b918e35) on 2026-05-08. CI + Vercel preview green. iOS WKAppBoundDomains + Capacitor allowNavigation set up for Clerk widget in WebView (B-medium). **A019.16:** ALL pages require login except `/sign-in`, `/sign-up`, `/privacy`, `/debug` — Discovery still free tier (no payment) but no longer anonymous (per-user logging in A021 needs guaranteed user_id). Web verified on Vercel preview. **Pending:** Phase 4 device verification (Capacitor Android + iPhone 15 — builds pre-staged, ready for Daniele to test), Railway env vars (`CLERK_*`) + Vercel env vars (`NEXT_PUBLIC_CLERK_*` including `*_FORCE_REDIRECT_URL` overrides). |
