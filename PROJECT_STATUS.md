...
## 🗓️ Ultimo Aggiornamento: 11 Maggio 2026 (B-A021-fix-1 shipped — Round 1 scroll + layout + Round 2 UI shrink + .pb-nav utility, re-validated on Vercel)
...
| A021 Climb logging + history (Phases 1–3) | ✅ Backend + Phase 3 frontend shipped — gym validation in progress | New tables: `climb_logs` (append-mostly source of truth, one row per `(user, climb, angle, local_date)`) + `user_climbs` (derived state cache: `is_project` + `best_result` + `last_logged_at`). Backend endpoints: `POST /api/logs` (rejects animated sequences, 422), `GET /api/logs` + `/api/logs/sessions`, `PATCH/DELETE /api/logs/{id}`, `GET/PATCH /api/user-climbs/{uuid}`, `GET /api/stats/{pyramid,trend}`. Search overlay: `GET /api/climbs/search` enriches each row with `user_state` when authenticated; new `done_filter`/`project_filter` chip params (`all`/`only`/`exclude`). All authenticated requests now carry `X-User-Timezone` (IANA) — backend resolves `local_date` via `Europe/Rome` or UTC fallback. Phase 3 frontend: `LogSection` (4 result buttons + project + remove) + `RecentLogs` mini-list mounted on `/discover/detail`; project-removal modal closes on Android back via `@capacitor/app`. Backend 217 tests, frontend 240 tests, all green. Phase 4 (chips + StateIcons on `/discover`) pending Daniele's iPhone gym validation. |
...
*Creato da Claude Code — 22 Febbraio 2026 | Aggiornato: 11 Maggio 2026*
