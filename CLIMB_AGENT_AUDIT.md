# Climb-Agent Audit — Reusable Patterns for Kilter-Up

> **Audited:** 2026-03-31
> **climb-agent path:** ~/Projects/climb-agent
> **Kilter-Up path:** ~/Projects/kilter-training-app
> **Type:** D004 (Read-Only Audit)

## Executive Summary

The climb-agent repo has mature developer tooling that Kilter-Up lacks: automated counter synchronization (`sync_status.py`), pre-push hook enforcement, a lessons-learned log (`docs/lessons.md`), repo hygiene checks, and a roadmap trimming script. The FastAPI architecture is structurally similar to Kilter-Up's, but climb-agent has a richer dependency injection layer (subscription guard, user resolution chain), a global exception handler, and a modular storage abstraction (file ↔ Supabase swap via env var). The most immediately portable items are the developer tooling scripts, the lessons.md pattern, and the global exception handler — all low-effort, high-value additions to Kilter-Up.
...
| **Global exception handler** | `backend/api/main.py` (lines 89-96) | Catches all unhandled exceptions, logs traceback, returns generic 500 | ~10 min — add to Kilter-Up's `main.py` |
| **Pre-push hook (counter sync)** | `.githooks/pre-push` | Blocks push if doc counters are stale; runs `sync_status.py` | ~30 min — adapt for Kilter-Up's simpler counter set |
| **Repo hygiene script** | `scripts/repo_hygiene.py` | Read-only diagnostic: stale docs, bloat, .DS_Store, large files | ~1 hr — adapt checks for Kilter-Up file structure |
| **Health check with persistence marker** | `backend/api/main.py` `/health` endpoint | Checks DATA_DIR writability, counts users, tests persistence survival | ~20 min — enhance Kilter-Up's existing `/health` |
...
| Pattern | Source File(s) | What It Does | Effort to Port |
|---------|---------------|--------------|----------------|
| **Counter sync script** | `scripts/sync_status.py` | Auto-updates test count, endpoint count, page count in PROJECT_BRIEF, README, CLAUDE.md via `<!-- STATUS_TABLE -->` markers | ~2 hr — adapt counters for Kilter-Up's stack |
...
| **Storage abstraction layer** | `backend/engine/storage.py`, `storage_file.py`, `storage_supabase.py` | Swap persistence backends via `STORAGE_BACKEND` env var; sys.modules replacement pattern | ~3 hr — useful when Kilter-Up moves to S3 |
...
| **Test isolation fixture** | `backend/tests/*.py` (autouse pattern) | `monkeypatch` + `tmp_path` per-test isolation; prevents cross-test contamination | ~1 hr — adapt for Kilter-Up's SQLAlchemy test DB |
| **Lifespan context manager** | `backend/api/main.py` (lines 68-71) | Validates DATA_DIR writable on startup before accepting requests | ~15 min — add startup checks to Kilter-Up |
| **Vocabulary governance doc** | `docs/vocabulary_v1.md` | Canonical closed-set definitions for all enums; new values must be added here first | ~1 hr — create for Kilter-Up's domain terms |
| **CLAUDE.md execution model** | `CLAUDE.md` | High-risk module list with mandatory analysis phase + STOP gate before implementation | ~30 min — adapt for Kilter-Up's critical modules |
...
| **Cluster-based exercise grouping** | `backend/engine/cluster_utils.py` | Normalizes attributes to stable keys for caching/tracking | ~1 hr — useful if Kilter-Up adds exercise recommendations |
...
| **Typed API client with retry** | `frontend/src/lib/api.ts` | Generic `request<T>()` wrapper with Clerk auth + single-retry on 401 | ~1 hr — reference for Kilter-Up's frontend API layer |
...
- **Portability:** Easy — adapt file whitelist and paths for Kilter-Up
...
- **Portability:** Medium — needs adaptation for Kilter-Up's different counters (videos, users, endpoints)
...
- **Portability:** N/A for Python linting (Kilter-Up already has its own setup)
- **Priority:** Low — Kilter-Up's existing setup is adequate

#### `.claude/settings.json` & `.claude/settings.local.json`
- **What:** Base permissions (Read-only) + extended local permissions (82 whitelisted: git, pytest, WebFetch, specific domains)
- **Portability:** Easy — reference for structuring Kilter-Up's Claude Code permissions
...
- **Priority:** Low — Kilter-Up already has test structure

#### `conftest.py`
- **What:** Minimal (8 lines) — only adds repo root to sys.path for imports
- **No global fixtures** — fixtures defined inline per test file
- **Portability:** N/A — Kilter-Up's conftest.py is already more comprehensive (DB fixtures, auth helpers)
- **Priority:** Low
...
- **Portability:** Medium — pattern translates to Kilter-Up's SQLAlchemy test DB isolation
...
- **Portability:** Easy — create fixture files for Kilter-Up's video analysis responses
- **Priority:** Medium
...
└── tests/                # 87 test files
```
- **Portability:** High — same FastAPI structure, Kilter-Up already uses similar layout
...
- **Portability:** Easy — add global handler to Kilter-Up's main.py
- **Priority:** High
...
- **Portability:** Easy — Kilter-Up already has CORS; no new middleware to port
- **Priority:** Low
...
- **Portability:** Easy — enhance Kilter-Up's `/health` with persistence check
...
- **Portability:** Already using same pattern in Kilter-Up
- **Priority:** Low
...
- **Portability:** Medium — useful when Kilter-Up migrates from SQLite to Postgres/S3
...
- **Portability:** N/A — Kilter-Up already uses Alembic properly
- **Priority:** N/A
...
- **Priority:** Low (Kilter-Up already does JSON repair on Gemini responses)
...
- **Portability:** N/A — Kilter-Up already uses Pydantic Settings (better pattern)
- **Priority:** N/A

#### Environment Variables
| Variable | Purpose | Kilter-Up Equivalent |
|----------|---------|---------------------|
...
- **Portability:** Easy — `ADMIN_SECRET` pattern useful for Kilter-Up admin endpoints
- **Priority:** Low
...
- **Portability:** Already similar in Kilter-Up (SQLite dev / Railway prod)
- **Priority:** Low
...
- User count
- **Portability:** Easy — add to Kilter-Up's `/health`
- **Priority:** Medium
...
- **Portability:** Already using same Railway setup in Kilter-Up
- **Priority:** Low
...
- **Portability:** Easy — add startup checks (DB connectivity, upload dir) to Kilter-Up
- **Priority:** Medium

#### Frontend: Vercel
- Auto-deploy on push to main; root: `frontend/`
- **Portability:** Same pattern available for Kilter-Up's Next.js frontend
- **Priority:** Low (Kilter-Up frontend deployment not yet active)
...
- **Portability:** Easy — add high-risk module list to Kilter-Up's CLAUDE.md
- **Priority:** Medium — useful as Kilter-Up grows in complexity
...
- **Priority:** Low (Kilter-Up focuses on form analysis, not training planning)
...
- **Portability:** Medium — good reference for Kilter-Up's frontend API layer
- **Priority:** Low (Kilter-Up frontend not yet mature)
...
- **Portability:** Easy — already partially adopted in Kilter-Up (B001, B003, D004)
...
| Pattern | Why Not |
|---------|---------|
| **File-based state storage** | Kilter-Up uses SQLAlchemy + PostgreSQL — proper relational DB is better |
| **sys.modules replacement for storage dispatch** | Clever but fragile; dependency injection via FastAPI `Depends()` is cleaner |
| **No linting/type-checking config** | Kilter-Up should have stricter tooling, not less |
...
| **No centralized Settings class** | Kilter-Up's Pydantic Settings is the better pattern |
| **Flat test directory (no unit/integration split)** | Kilter-Up benefits from organized test categories |

---

## Recommended Next Steps

1. **Create `docs/lessons.md`** in Kilter-Up — start accumulating learnings immediately (5 min)
...
5. **Create high-risk module list** in CLAUDE.md — identify Kilter-Up modules requiring analysis-first protocol (30 min)
6. **Set up `.githooks/pre-push`** — enforce test pass before push (adapt for `pytest` in Kilter-Up) (30 min)
7. **Create `scripts/sync_status.py`** — auto-update test count, endpoint count in docs (2 hr)
8. **Create `scripts/repo_hygiene.py`** — adapt for Kilter-Up's file structure (1 hr)
