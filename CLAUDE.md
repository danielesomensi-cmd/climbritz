...
│   └── layout.tsx (next/font: Space Grotesk + Inter Tight, self-hosted; mounts IosSafeArea) · globals.css (Tailwind v4 + B033 `@theme static` design tokens + B034 `--safe-top`) · safe-area.css (.pt-safe/.pb-safe/.pb-nav — all read `--safe-top`)
├── components/         BoardMap · ClimbBoardView · ClimbCard · StateIcons · LogSection · RecentLogs · FilterPanel · GradeDisplay · StarRating · BottomNav · ClimbBleControls · ClerkSpaProvider · AuthGuard · AuthShell · IosSafeArea (B034 native-iOS safe-area probe)
