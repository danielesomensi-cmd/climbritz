# 🧗 Claude Code Guidelines - Climbritz App

## Project Overview
**Climbritz** — AI-powered climbing form analysis app.
...
✅ **A022:** Benchmark filter on `/discover` — new boolean "Benchmarks only" toggle at the top of `FilterPanel`. Backend: optional `benchmark` query param on `GET /api/climbs/search` → adds `AND cs.benchmark_difficulty IS NOT NULL` to the shared `_build_search_filters()` helper (so both `search_climbs` and `count_matching_climbs` respect it). **Angle-specific by DB design:** `benchmark_difficulty` lives on `climb_stats` keyed by `(climb_uuid, angle)`, and Discovery always sends `angle`, so the filter means "benchmark at the currently selected angle". Phase 0.5 read-only audit confirmed the model empirically — 93.4% of the 412 benchmark climbs are validated at ≤3 angles (max 5, none at all 11), and a climb's grade itself varies per angle (e.g. *swooped*: 5b/V1 at 5° → 7a+/V7 at 50°, benchmark only at 40° and 50°). A single boolean (not tri-state) — "exclude benchmarks" isn't a real user need. `countActiveFilters()` counts an active toggle; URL param + sessionStorage forward-compat. Test fixtures seeded by `backend/scripts/seed_a022_test_fixtures.py` (idempotent) — Alpha@40 + Crimp@40 benchmarked, Alpha@45 NULL to prove angle-specificity. Backend 229 tests, frontend 302 tests — all green, web + mobile builds clean.
...
│   │   ├── seed_a019_test_fixtures.py ✅ A019 — idempotent moves-bucket fixture seeder
│   │   └── seed_a022_test_fixtures.py ✅ A022 — idempotent benchmark fixture seeder (sets benchmark_difficulty on chosen fixture rows)
│   ├── conftest.py                 ✅
│   └── requirements.txt
...
│   ├── FilterPanel.tsx             ✅ Discovery filters (A011 + A021.4 doneFilter/projectFilter on Filters type + A022 benchmark toggle)
...
├── capacitor.config.ts             ✅ Capacitor config (appId=app.climbritz, webDir=out)
...
**Version:** 2.38 (B021 iOS completion — **Clerk production now works on iOS too**, after a long night of platform-specific gotchas. Key findings: (1) iOS/WKWebView **ignores `iosScheme:'https'`** (reserves the https scheme), so the iOS WebView origin is **`capacitor://app.climbritz.app`**, NOT `https://app.climbritz.app` like Android — the `hostname` is honored but the scheme stays `capacitor://`. (2) **`WKAppBoundDomains` removed** from `Info.plist` (`f435516`): it doesn't cover subdomains, so the app's own origin was non-app-bound → WKWebView refused user-script injection → clerk-js never loaded → spinner. Not needed for email/password. (3) **Two separate allowlists** must each carry **both** per-platform origins: Clerk `allowed_origins` (else `origin_invalid` 400 → spinner) AND backend CORS in `main.py` (`58151d1`, else authenticated API calls fail — that was the "Import failed" on `/classify`). Both now list `https://app.climbritz.app` + `capacitor://app.climbritz.app` + the three localhost variants. iOS verified end-to-end on iPhone (Xcode debug build): login + session + Import JSON + Discover. **New runbook `docs/CLERK_CAPACITOR_AUTH.md`** captures the 3-gate model (Clerk origin / session cookie / backend CORS), the per-platform origin table, and the debugging playbook (Android=adb fully inspectable; iOS=Xcode debug + Safari Web Inspector, TestFlight not inspectable). prev 2.37: B021 — **Clerk Development → Production promotion + email/password auth**: promoted Clerk to the production instance on custom domain `clerk.climbritz.app` (`pk_live`/`sk_live`; backend JWKS issuer → `clerk.climbritz.app`, env-driven in `clerk.py` — no code change there). Auth model **decided: email + password, with the 6-digit email code only at sign-up** (deviates from the brief's original "email-code-only" — the WebView blockers proved solvable and password decouples daily login from flaky new-domain email delivery). Google OAuth + magic links disabled. **The hard bug:** production Clerk sets its session cookie `SameSite=Lax` on `Domain=climbritz.app`; the Capacitor WebView ran on origin `https://localhost` (Android) / `capacitor://localhost` (iOS) → cross-site → cookie never sent → clerk-js showed "You are signed out" right after a correct sign-in (dev `pk_test` used a URL token, so dev had worked). **Fix:** `capacitor.config.ts` `server.hostname = app.climbritz.app` + `androidScheme`/`iosScheme = https` → WebView origin is a `climbritz.app` subdomain → cookie is same-site → session persists (also natively satisfies the `pk_live` domain-lock). Along the way: Clerk `allowed_origins` set (localhost variants + `app.climbritz.app`) to let clerk-js load; **Bot protection / Turnstile CAPTCHA disabled** (it failed in the WebView with `captcha_missing_token`, blocking sign-up before any OTP send); backend CORS now allows `https://app.climbritz.app` (`backend/app/main.py`). iOS `Info.plist` `WKAppBoundDomains` → `climbritz.app` + `clerk.climbritz.app` + `accounts.climbritz.app` (was the stale dev domain). **Android build 8** (versionCode 8, tag `android-v1.0.0-build8`): release APK login + cold-start persistence + authenticated Discover load all verified on-device (Galaxy Tab A11); signed AAB ready for Play Console. **iOS rebuild pending** — run `bash ~/deploy_climbritz.sh 4` from Terminal.app (same config covers iOS). Full root-cause writeup in the `project-clerk-prod-capacitor-origin` memory. prev 2.36: Android build6 — **MainActivity package fix**: build4/5 crashed at launch with `ClassNotFoundException: app.climbritz.MainActivity` because the MainActivity source sat at legacy package `com.kilterup.app` while `namespace = app.climbritz` (AGP 8 resolves manifest `.MainActivity` against the namespace). Latent since A006, never device-tested. Moved source to `app.climbritz` (`1909da2`), versionCode 6, tag `android-v1.0.0-build6`, on-device verified. Android build7 — **Climbritz launcher icon** (B-android-icons): default Capacitor robot replaced with the board icon generated from iOS `AppIcon-1024.png` via `@capacitor/assets` (adaptive bg `#0f1e36`); capacitor-assets' splash/iOS/manifest side effects reverted; `assets/play-store-icon-512.png` generated for the store listing. versionCode 7, tag `android-v1.0.0-build7`, on-device verified — 2026-05-29. prev 2.35: A023 — Hold Classification Cloud Sync + Crowdsource Growth: `/classify` moved off localStorage-only onto a per-user backend table (`user_hold_classifications`, alembic 005) so verdicts sync across devices under the same Clerk identity. New `/api/classifications` surface (GET list, PUT upsert, DELETE idempotent, POST import merge-upsert), all Clerk-gated. No x/y columns — coords joined frontend-side from `placements_12x12.json` at export, same as before. `/classify` gains backend hydration (write-through localStorage cache, no auto-migration), optimistic upsert/delete with rollback toast, two growth banners, a "Send my export" button (clipboard + mailto to daniele.somensi@gmail.com) and an "Import JSON" button (Daniele's bootstrap path — same path every user uses). Per-user only; cross-user aggregation into a canonical map stays the future HC-7 brief. Backend 231 + frontend 307 green; tsc + web + mobile builds clean — 2026-05-28)
