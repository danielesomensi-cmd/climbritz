...
**BLE (Phase 3e):** runtime path is `lib/ble/transport.ts` (wrapper over `@capacitor-community/bluetooth-le`) + `lib/ble/kilter-board-service.ts` (Kilter orchestration + UUIDs) + `lib/ble/kilter-protocol.ts` (pure packet encoder, API level 3). Connection state is **global** (B035): `components/BleProvider.tsx`, mounted in the root layout, owns the single state machine (8 states + promise-chain write serialization); pages consume it via `useBle()` — connect once, stay connected across navigation and app background. Contract: never disconnect except on explicit user tap; **never auto-reconnect** (gym etiquette — manual tap only); app resume runs a read-only `getConnectedDevices` check to re-sync the UI. Web Bluetooth / Grip Connect are NOT on the runtime path — `@hangtime/grip-connect` stays in `package.json` as protocol reference only (never imported). `UnsupportedBoardError` for API level 2 boards. Protocol docs: https://github.com/1-max-1/fake_kilter_board
...
- **Coach tier** — Video Analysis (L1) is live and the homepage tile is a normal entry point. The old "Coming Soon" pill was removed in A-STORE-PROD-001 (factually wrong + an App Store Guideline 2.1 trigger). No pricing or payment surface exists anywhere in the app — keep it that way until an IAP exists, or Guideline 3.1.1 becomes an automatic rejection.
- **Account deletion** — `/settings` → `Delete account` (Guideline 5.1.1(v)), one tap from home via the ⚙️ beside the avatar.

- **Problem Generation (Phase 3f) ✅ v1 + save** — `/generate` page + `POST /api/climbs/generate` (A026). Remix a real climb from the filtered pool: swap ≥2 hand holds (same-role, corpus-derived ε + y-bands) → light it up over BLE. **A030:** one-tap save → "My Problems" (`/my-problems`, AI-proposed name, grade from seed, loggable, merged into Discovery with a MY badge); grip-type filtering / grade-coherence / hold editor are future.
...
- `GET /api/climbs/search?q=&angle=&grade_min=&grade_max=&min_ascents=&min_quality=&moves=&benchmark=&nomatch=&my_problems=&sort=&limit=&done_filter=&project_filter=`
  - `moves` bucket: `any` | `le5` | `6-7` | `8-10` | `gt10`. `benchmark` (bool): climbs flagged at the selected angle (angle-specific by DB design). `nomatch` (bool, A029): only climbs with the setter's "no matching" rule (`climbs.is_nomatch`, climb-level — audit D019: flag is authoritative, no description heuristics); search results and the detail payload both carry `is_nomatch`. Animated sequences (`frames_count > 1`) excluded globally. `limit` defaults to 500, hard cap 500 (422 above). Response: `{climbs, total_count}` (total_count ignores the cap → drives the overflow banner).
  - When authenticated: also accepts `done_filter`/`project_filter` (`all`|`only`|`exclude`) and enriches each climb with `user_state` `{is_project, best_result, last_logged_at}`. **A030:** `my_problems` (`include` default | `only`) merges the user's saved generated problems — prepended, flagged `is_mine: true`, setter "You", `quality_average: null` (schema is `float|None`), ascents = own flash/send count; `only` skips BoardLib. Filters are applied with equivalent semantics Python-side; benchmark/nomatch/min_quality naturally exclude mine.
...
│   ├── settings/       page (A-STORE-PROD-001 — Delete account: 2-step confirm, type-DELETE, clears web storage + Clerk signOut + hard redirect. Reached in 1 tap via the ⚙️ next to the avatar on `/`)
...
│   └── layout.tsx (next/font: Space Grotesk + Inter Tight, self-hosted; mounts IosSafeArea + BleProvider) · globals.css (Tailwind v4 + B033 `@theme static` design tokens + B034/B036 `--safe-top` = max(`env()`, Capacitor-SystemBars-injected `--safe-area-inset-top`) for Android edge-to-edge) · safe-area.css (.pt-safe reads `--safe-top`; .pb-safe/.pb-nav max() env() with the injected bottom var directly — no token, see B036 note)
├── components/         BoardMap · ClimbBoardView · ClimbCard · StateIcons · LogSection · RecentLogs · FilterPanel · GradeDisplay · StarRating · BottomNav · ClimbBleControls · BleProvider (B035 global BLE state + useBle()) · ClerkSpaProvider · AuthGuard · AuthShell · IosSafeArea (B034 native-iOS safe-area probe)
