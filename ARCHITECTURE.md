...
| **Generate API** | `api/generate.py` | A026 — `POST /api/climbs/generate` (JWT). Thin wiring: builds the candidate pool via the same `_build_search_filters` path as Discovery (`min_ascents=5`), then hands injected hold data to the pure `problem_generator.remix`. 422 on pool-too-small / can't-reach-2-swaps. Ephemeral — nothing persisted. |
