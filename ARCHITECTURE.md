...
> Last updated: 5 June 2026 (through A026 AI Problem Generator / Remix v1)
...
| **Climb API** | `api/climbs.py` | Search, detail, stats (BoardLib DB queries). Search excludes animated sequences (`frames_count > 1`) globally; A019 `moves` query param applies a SQL WHERE on the cyan-hold count. **A022:** `benchmark` bool param adds `AND cs.benchmark_difficulty IS NOT NULL` — angle-specific, since the `cs` join is already constrained to the selected angle. **A029:** `nomatch` bool param adds `AND c.is_nomatch = 1` (climb-level setter rule, audit D019 — flag is authoritative, no description heuristics); `is_nomatch` is included in every search result and in the detail payload. B020: `limit` defaults to 500 with hard cap 500 (422 above); response is the envelope `{climbs, total_count}` so the UI can surface an overflow banner without a second request — `total_count` reflects all matching climbs ignoring the cap. **A021.4 overlay:** when the request is authenticated, search now also accepts `done_filter` and `project_filter` chip params (`all`/`only`/`exclude`) and enriches each climb with a `user_state` object. The overlay is a Python merge (Option A) — BoardLib lives in a separate SQLite file from the app DB, so the API layer pre-fetches matching uuids from `user_climbs` and threads them as include/exclude lists into `_build_search_filters`. |
| **Generate API** | `api/generate.py` | A026 — `POST /api/climbs/generate` (JWT). Thin wiring: builds the candidate pool via the same `_build_search_filters` path as Discovery (`min_ascents=5`), then hands injected hold data to the pure `problem_generator.remix`. 422 on pool-too-small / can't-reach-2-swaps. Ephemeral — nothing persisted (saving is A030's `/api/my-climbs`). |
| **My Problems API** | `api/my_climbs.py` | A030 — CRUD for saved AI-generated problems (JWT, owner-scoped: cross-user → 404). POST validates the exact BoardLib `p{id}r{role}` frames encoding (roles {12,13,14,15}, no dup placements), prefills grade/difficulty from the seed's BoardLib stats and names the problem via `problem_name_service`. Storage `user_generated_climbs` (migration 006) is BoardLib-compatible by design (`is_listed` = future publishing flag flip, `is_nomatch` = A029 parity). Discovery merge: `my_problems=include` (default) prepends the user's problems flagged `is_mine` Python-side (Option A, BoardLib query untouched; `quality_average` → `float\|None`); `only` skips BoardLib. `POST /api/logs` branches to an owner-scoped `user_generated_climbs` lookup before the BoardLib guard; the sessions resolver falls back there for name/grade. **A031:** `POST /propose-name` (name-on-generate, `{name, source}`), optional `name`/`grade` overrides on POST, `frames` editable via PATCH (re-validated; logs stay attached), structural rules enforced everywhere (1–2 start, 1–2 finish, ≥3 holds). |
| **Problem naming** | `services/problem_name_service.py` | A030 — Gemini 2.5-flash text-only (`thinking_budget=0`, ~50 tokens, 2s http timeout) proposes one witty 2–4 word name; ANY failure → local adjective+noun fallback. Separate module (A025 precedent): reuses `gemini_service._get_client`, the high-risk video module is untouched. Saving never fails or blocks on naming. |
...
| **Climb Service** | `services/climb_service.py` | Read-only sqlite3 queries against BoardLib DB. A021: `get_climb_meta()` for the POST-/api/logs animated-sequence guard; `include_uuids` / `exclude_uuids` params on `_build_search_filters` for the Discovery chip-filter pipeline. A026: `get_holds_for_climbs()` bulk-fetches parsed holds + positions for the generator pool (2 queries regardless of pool size). |
| **Problem Generator** | `services/problem_generator.py` | A026 — pure, DB-free remix logic. `remix(pool, config, rng)` picks a seed (weighted by ascents), swaps ≥2 same-role hand holds with candidates within ε of the seed inside role y-bands. `GeneratorConfig` holds all tunables (corpus-derived in Phase 0: ε=24, start_y_max=88, finish_y_min=144, min_ascents=5, K=10). `PoolTooSmallError` / `CannotGenerateError` → 422. Injectable `rng` for deterministic tests. |
...
| Generate | `generate/page.tsx` | A026 — AI problem generator (Remix v1). Query-param route, AuthGuard-wrapped. Filters (angle / grade range / moves; grip-type disabled "Coming soon"; stars + benchmark omitted) → `generateProblem()` → renders the remix via `ClimbBoardView` + lights it up via `ClimbBleControls` (the `{placement_id, role}` output maps straight through `climb-to-leds`). "Generate again" re-rolls. **A031:** the section is labeled "AI Create"; every roll fires `/propose-name` in parallel (board never waits; stale guard on the generation counter) and shows the name big above the board; "Save as is" persists the displayed name; "Edit" / "Start from blank" hand a draft to the `/create` hold editor via sessionStorage (`climbritz:create:draft`, 1h TTL — never the query string). The editor (`app/create/`) paints roles on `BoardMap` (palette colors from `kilter-protocol.PLACEMENT_ROLES`; bolt-ons take all roles per real-corpus semantics, screw-ons Foot only), live-sends LEDs debounced 300ms while connected, and mirrors the server validation client-side. |
...
| **History** | `history/page.tsx` | A021.5 + A024 + A025 — single-scroll page: sticky date range picker (7d/30d/90d/1y/All, default 90d) + A025 AI progress-comment card (on-tap, free; `coach-comment.tsx` → `POST /api/coach/summary`) + stats header (A024 + B033: uniform 2+3 grid — Climbs volume / Sessions / Sends / Flashes / Peak grade; Climbs summed from the sessions payload, no extra endpoint. B033: SENDS = send-or-better, i.e. plain sends + flashes — a flash is a send; FLASHES is the flash-only subset) + sessions list (cards desc, climb rows link to `/discover/detail`) + Recharts horizontal-bar grade pyramid (flash/send_or_better/all filter) + Recharts line trend chart (per-series toggle for flash/send/attempt). A024 dropped the hand-rolled calendar heatmap. Recharts 3.8.1 pulled in for the two chart sections. |
...
```
app/ble-test/page.tsx + ClimbBleControls   UI — preset grid / climb controls, auto-apply debounce, error banner
  └─ components/BleProvider.tsx  Global React context (B035, mounted in the root layout) — owns the single
       │                         state machine (8 states) + promise-chain write serialization; consumed via useBle()
...
**B035 — persistent connection across navigation:** BLE state used to live in page-level components (one `useKilterBle()` state machine per page) so navigating away dropped the connection. `BleProvider` lifts the single state machine to the root layout (stable across SPA navigation, same pattern that keeps ClerkProvider alive): connect once, stay connected anywhere in the app and across app background. Behavior contract: (1) never actively disconnect except on the user's explicit Disconnect tap — no disconnect on route change, unmount, or background; (2) **never auto-reconnect** — if the OS or another user (board takeover) drops the link, reconnection is a manual tap only (gym etiquette: auto-reconnect on resume would kick off whoever connected in the meantime); (3) UI always reflects true state — the transport `onDisconnect` callback updates the shared state on live drops, and on app resume a strictly read-only `BleClient.getConnectedDevices` check re-syncs the UI.
...
| Safe-area CSS (B017) | Plain utilities in `app/safe-area.css`, imported separately from `globals.css` | Tailwind v4's postcss pipeline strips plain class rules from the Tailwind entrypoint (even inside `@utility`/`@layer`). Separate CSS import survives. `.pt-safe`/`.pb-safe`/`.pb-nav` read the `--safe-top`/`--safe-bottom` tokens (globals.css); `viewport-fit=cover` set via Next `viewport` export in `app/layout.tsx` |
| Safe-area insets on Android (B036) | Every safe-area rule = `max(env(safe-area-inset-*), var(--safe-area-inset-*))` | targetSdk 36 → Android 15+ enforces edge-to-edge and the WebView's `env()` is unreliable (0 on Samsung S22/S24 → header under the status bar). Capacitor 8's built-in SystemBars plugin injects the real insets as `--safe-area-inset-*` on `<html>`; `--safe-top` (globals.css) and the bottom utilities (safe-area.css) max() both sources. Bottom stays env()-direct at the usage site (no `--safe-bottom` token) per B034's iOS env()-through-custom-property caution. iOS unaffected: `IosSafeArea.tsx` overrides `--safe-top` inline on native iOS (beats the `:root` rule); on web the injected vars don't exist → 0px fallback. `SystemBars: { style: 'DARK' }` in `capacitor.config.ts` keeps status-bar icons light over the dark app (Android-only option). Verified live on an API-37 emulator via CDP (injected top=44px/bottom=24px, `--safe-top`→45px) |
