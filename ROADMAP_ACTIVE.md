...
- [x] **A021 Climb logging + history (Phases 1–3)** — new `climb_logs` + `user_climbs` tables (alembic 004), endpoints under `/api/logs`, `/api/user-climbs`, `/api/stats`. Search overlay enriches each result with `user_state` when authenticated; `done_filter` / `project_filter` chip params live (Phase 4 wires the UI). Frontend Phase 3 shipped: `LogSection` (5-col single row: Flash / Send / Attempt / Project / Remove) + `RecentLogs` mini-list on `/discover/detail`; project-removal modal closes on Android back via `@capacitor/app`. All authenticated requests carry `X-User-Timezone` (IANA). Backend 217 tests, frontend 242 tests.
